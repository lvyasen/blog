---
title: Go Protobuf
urlname: eorhvpufhdyluopk
date: '2022-12-31 10:14:35 +0800'
tags: []
categories: []
---
...
> 进入 [protobuf 仓库](https://github.com/protocolbuffers/protobuf/releases)
...
> 有些 google 包不能下载。
> 进入 `GOROOT`目录，创建文件夹 `google.golang.org `下载 [protobuf](https://gitcode.net/mirrors/protocolbuffers/protobuf-go?utm_source=csdn_github_accelerator) 包，将文件名改为 `protobuf`

```bash
git clone https://gitcode.net/mirrors/protocolbuffers/protobuf-go.git
```
...
package progobuf_test

import (
	"github.com/golang/protobuf/proto"
	progobuf "go-test/progobuf_test"
	"testing"
)

func TestUser_ProtoMessage(t *testing.T) {
	user := progobuf.User{
		UserId:               11,
		Username:             "张三",
	}
	...
	t.Log(userByte)//打印二进制消息
	userRes := progobuf.User{}
	//将 byte 反序列化成 json
	err2:=proto.Unmarshal(userByte,&userRes)
	if err2 != nil {
		t.Logf("反序列化失败,%s",err2)
	}
	t.Logf("userRes: %v",userRes)
}
```
